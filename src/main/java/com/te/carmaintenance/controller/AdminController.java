...
public class AdminController {
	...
	@PostMapping("/login")
	public ResponseEntity<?> loginAuthenticationToken(@RequestBody AdminRequest adminRequest) {
		try {
			authenticationManager.authenticate(
					new UsernamePasswordAuthenticationToken(adminRequest.getUsername(), adminRequest.getPassword()));
		} catch (AuthenticationException e) {
			return ResponseEntity.ok(new AdminResponse("invalid username and password", true, null));
		}
		...
		return ResponseEntity.ok(new AdminResponse("Authentication success", false, jwtToken));
	...
	public ResponseEntity<?> signUpAuthentication(@RequestBody Admin admin) {
		...
		if (signUpData != null) {
			...
			String jwtToken = jwtUtil.generateToken(details);
			return ResponseEntity.ok(new AdminResponse("signUp success", false, null));

		} else {
			return ResponseEntity.ok(new AdminResponse("username already exists", true, null));
		}
	...
	public ResponseEntity<?> getAllCarDetails() {
		System.out.println("In getAllCarDetails()!");
		try {
